...
public class Histogram {
  ...
  public void buildHistogram(Table table, int attribute){
      ...
      // TODO: HW4 implement
      Iterator<RecordId> ridIter = null;
      Record recordTemp = null;
      ridIter = table.ridIterator();
      float minHis = 0f;
      float maxHis = 0f;
      float width = 0f;
      float temp;
      ...
      while(ridIter.hasNext()){
          try {
              recordTemp = table.getRecord(ridIter.next());
          }catch (DatabaseException e){
              break;
          }
          temp = quantization(recordTemp,attribute);
          if (temp > maxHis){
              maxHis = temp;
          }
          if (temp < minHis){
      ...
      width = (maxHis - minHis) / numBuckets;
      //3. create each bucket object
      //Bucket<Float> [] bucketsTemp;
      for(int i = 0; i < numBuckets;i++){

          buckets[i] = new Bucket(minHis + width * i, minHis + width * (i+1));
      ...
      while(ridIter.hasNext()){
          ...
          temp = quantization(recordTemp,attribute);
          if (width == 0f){
              buckets[numBuckets - 1].increment(temp);
              continue;
          }
          if (temp == maxHis){
              buckets[numBuckets - 1].increment(temp);
              ...
              int ind = (int) ((temp - minHis) / width);
              buckets[ind].increment(temp);
          }
      }
  }
  ...
  private float [] allEquality(float qvalue){
    float [] result = new float[this.numBuckets];
    float minHis = buckets[0].getStart();
    float maxHis = buckets[this.numBuckets - 1].getEnd();
    float width = buckets[0].getEnd() - buckets[0].getStart();
    int ind;
    int distinctCount = 0;
    if (width == 0){
        ind = this.numBuckets - 1;
    }
    else{
        if (qvalue == maxHis){
            ind = this.numBuckets - 1;
        }
        else if(qvalue < maxHis && qvalue >= minHis){
            ind = (int) ((qvalue - minHis)/width);
        }
        else{
            ind =this.numBuckets;
        }
    }
    if(ind < this.numBuckets){
        distinctCount = buckets[ind].getDistinctCount();
    }
    for (int i = 0;i < this.numBuckets;i++){
        if (i == ind){
            result[i] = 1f/(float) distinctCount;
            continue;
        }
        result[i] = 0.0f;
    }
 ...
 /**
  ...
  private float [] allNotEquality(float qvalue){
    float [] result = new float[this.numBuckets];
    float minHis = buckets[0].getStart();
    float maxHis = buckets[numBuckets - 1].getEnd();
    ...
    for (int i = 0;i < this.numBuckets;i++){
      if (i == ind){
          result[i] = 1f - 1f/(float) distinctCount;
          continue;
      }
      result[i] = 1.0f;
    }
  ...
  private float [] allGreaterThan(float qvalue){
    ...
    for (int i = 0;i < this.numBuckets;i++){
      if (i < ind){
          ...
          result[i] = (buckets[ind].getEnd() - qvalue)/width;
      }
    }
  ...
  private float [] allLessThan(float qvalue){
    ...
    for (int i = 0;i < this.numBuckets;i++){
      if (i < ind){
          ...
          result[i] = (qvalue - this.buckets[ind].getStart())/width;
      }
    }
    // TODO: HW4 implement;
